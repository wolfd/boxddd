...
static int TestZeroedAllocate( void )
{
	ENSURE( b3AllocZeroed( 0 ) == NULL );

	uint32_t* values = b3AllocZeroed( 4 * sizeof( uint32_t ) );
	ENSURE( values != NULL );
	for ( int i = 0; i < 4; ++i )
	{
		ENSURE( values[i] == 0 );
	}
	b3Free( values, 4 * sizeof( uint32_t ) );
...
{
	RUN_SUBTEST( TestZeroedAllocate );
