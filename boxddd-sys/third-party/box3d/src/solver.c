	...
	if ( workerIndex == 0 )
	{
		...
		*/
		...
		for ( int subStepIndex = 0; subStepIndex < subStepCount; ++subStepIndex )
		{
			...
			// Warm start constraints. The overflow contact warm start now rides a
			// parallel per-island stage below (published through b3PublishSyncBits inside
			// b3ExecuteMainStage); only the joint overflow stays serial, so the
			// serial-phase hint brackets just that call and workers join the island stage.
			b3BeginSerialPhase( context );
			...
			for ( int j = 0; j < ITERATIONS; ++j )
			{
				...
				// Overflow contacts solve in the parallel per-island stage; only the joint
				// overflow stays serial inside the serial-phase hint.
				b3BeginSerialPhase( context );
			...
			{
				// Overflow contacts relax in the parallel per-island stage; only the joint
				// overflow stays serial inside the serial-phase hint.
				b3BeginSerialPhase( context );
				b3SolveJoints_Overflow( context, useBias );
				b3EndSerialPhase( context );
		...
		// Restitution. The overflow restitution now rides the parallel per-island stage,
		// so there is no serial window (and no serial-phase hint) here anymore.
		{
