...
uint32_t b3RecInternVoxel( b3Recording* rec, const b3VoxelData* voxel )
{
	int byteCount = 0;
	uint8_t* bytes = b3Voxel_Serialize( voxel, &byteCount );
	B3_ASSERT( bytes != NULL && byteCount > 0 );
	uint64_t h = b3Hash64NonZero( bytes, byteCount );
	return b3InternGeometry( &rec->registry, b3_geometryVoxel, h, bytes, byteCount );
}
