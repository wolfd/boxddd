...
impl BodySleepData {
    #[inline]
    pub(crate) fn from_raw(raw: ffi::b3BodySleepData) -> Self {
        Self {
...
impl IslandCensus {
    #[inline]
    pub(crate) fn from_raw(raw: ffi::b3IslandCensus) -> Self {
        Self {
            island_id: raw.islandId,
            ...
            min_sleep_time_body: BodyId::from_raw(raw.minSleepTimeBody),
            sleep_ready_count: raw.sleepReadyCount,
        }
    }
}

/// Reusable buffer for [`World::try_world_body_sleep_buffered`], following
/// the allocation-free-once-warm pattern of [`ContactBuffer`].
///
/// [`World::try_world_body_sleep_buffered`]: crate::World::try_world_body_sleep_buffered
#[derive(Default)]
...
impl BodySleepBuffer {
    ...
    /// into world storage.
    pub(crate) fn convert_raw(&mut self) {
        self.entries.clear();
        self.entries.reserve(self.raw.len());
        self.entries
            .extend(self.raw.iter().map(|raw| BodySleepData::from_raw(*raw)));
    }
}

/// Reusable buffer for [`World::try_world_island_census_buffered`],
/// following the allocation-free-once-warm pattern of [`ContactBuffer`].
///
/// [`World::try_world_island_census_buffered`]: crate::World::try_world_island_census_buffered
#[derive(Default)]
...
impl IslandCensusBuffer {
    ...
    pub(crate) fn convert_raw(&mut self) {
        ...
        self.entries.reserve(self.raw.len());
        self.entries
            .extend(self.raw.iter().map(|raw| IslandCensus::from_raw(*raw)));
    }
}
