...
use boxddd::{
    Aabb, BodyDef, BodyType, BoxHull, QueryFilter, ShapeDef, ShapeId, Vec3, World, WorldDef,
};
...
fn assert_fat_is_the_tree_bound(world: &World, shape: ShapeId, label: &str) {
    let tight = world.try_shape_aabb(shape).unwrap();
    let fat = world.try_shape_fat_aabb(shape).unwrap();
...
fn fat_aabb_is_the_broad_phase_bound_through_create_move_and_enlarge() {
    ...
    world
        .try_set_body_transform(body, [3.0, 5.0, -2.0], boxddd::Quat::IDENTITY)
        .unwrap();
